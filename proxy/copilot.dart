...
void main() {
  print("AI Proxy Plugin Loaded (Copilot).");
  initWebSocket();
}
...
void initWebSocket() {
  ...
  final getCallback = ((JSObject result) {
    ...
    ws = web.WebSocket('$wsUrl?model=copilot');
...
Future<String?> reqAI(String prompt) async {
  try {
    final inputElement = web.document.querySelector('#userInput');
    ...
    final initialCount =
        web.document.querySelectorAll('[data-testid="ai-message"]').length;
    ...
    for (int i = 0; i < maxAttempts; i++) {
      final responseList =
          web.document.querySelectorAll('[data-testid="ai-message"]');
