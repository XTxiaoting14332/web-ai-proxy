...
Future<void> reqAIStream(String prompt) async {
  try {
    ...
    for (int i = 0; i < maxAttempts; i++) {
      final responseList = web.document.querySelectorAll('[data-message-author-role="assistant"]');
      String currentText = "";
      if (responseList.length > 0) {
        final el = responseList.item(responseList.length - 1) as web.HTMLElement;
        final mc = el.querySelector('.markdown');
        currentText = mc != null ? (mc as web.HTMLElement).innerText.trim() : el.innerText.trim();
      }
